...
export async function initialize(workspaceDir: string): Promise<StartupResult> {
  ...
  try {
    ...
    log.error(
      "database connection failed — cannot start without a database",
      error,
    );
    log.error(
      "check DATABASE_URL in .env and ensure the database file is accessible",
    );
    process.exit(1);
  }
  ...
  if (config.KNOWLEDGE_BASE_ENABLED) {
    void loadEDITHConfig()
      ...
      .catch((err) =>
        log.warn("knowledge base sync scheduler init failed", { err }),
      );
  }
  ...
  try {
    ...
    const edithJson = await fs
      .readFile(edithJsonPath, "utf-8")
      .catch(() => "{}");
    const edithConfig = JSON.parse(edithJson) as Record<string, unknown>;
    const mcpServers: MCPServerConfig[] =
      (edithConfig?.mcp as { servers?: MCPServerConfig[] })?.servers || [];
  ...
  const shutdown = async (): Promise<void> => {
    ...
    await mcpClient
      .shutdown()
      .catch((err) => log.warn("MCP shutdown error", err));

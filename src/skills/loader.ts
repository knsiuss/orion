...
const SKILL_DIR_BUNDLED = path.resolve(
  process.cwd(),
  "src",
  "skills",
  "bundled",
...
function parseFrontmatter(skillMdContent: string): ParsedFrontmatter | null {
  ...
  try {
    ...
    log.debug("failed to parse skill frontmatter YAML", { err });
    return null;
  }
}
...
export class SkillLoader {
  ...
  async buildSnapshot(): Promise<SkillSnapshot> {
    ...
    for (const dir of SKILL_DIRS_BY_PRECEDENCE) {
      ...
      for (const entry of sortDirents(entries)) {
        ...
        const discovered = await this.discoverSkillFromDirectory(
          dir,
          entry,
          platform,
          seen,
    ...
    this.snapshot = {
      ...
      alwaysActiveContent:
        await this.buildAlwaysActiveContent(alwaysActiveSkills),
    };
  ...
  async getIndexForPrompt(
    options: SkillEligibilityOptions = {},
  ): Promise<string> {
  ...
  async getAlwaysActiveContent(
    options: SkillEligibilityOptions = {},
  ): Promise<string> {
    const snapshot = await this.getSnapshot();
    const eligible = this.filterByToolPolicy(snapshot.skills, options);
    return this.buildAlwaysActiveContent(
      eligible.filter((skill) => skill.alwaysActive),
    );
  }
  ...
  async loadSkillContent(location: string): Promise<string | null> {
    ...
    try {
      ...
      log.debug("failed to read skill content file", {
  ...
  stopWatching(): void {
    for (const watcher of this.watchers) {
      try {
        ...
        log.debug("failed to close skill file watcher", { err });
        continue;
      }
    }
  ...
  private async readDirectoryEntries(dir: string): Promise<Dirent[]> {
    try {
      ...
      log.debug("skill directory not accessible, skipping", { dir, err });
      return [];
    }
  }
  ...
  private async discoverSkillFromDirectory(
    ...
    try {
      ...
      log.debug("SKILL.md not found or unreadable", { skillMdPath, err });
      return null;
    }
  ...
  private buildSkillMeta(
    ...
    const invokeKey = parsed.invokeKey?.trim().length
    ...
    return {
      ...
      version: parsed.version?.trim().length
        ? parsed.version.trim()
        : undefined,
      enabled: true,
    };
  ...
  private createWatcher(dir: string): FSWatcher | null {
    try {
      ...
      log.debug(
        "recursive watch not supported, falling back to non-recursive",
        { dir, err },
      );
      try {
        return watch(dir, () => this.scheduleSnapshotRefresh());
      } catch (innerErr) {
        log.debug("file watcher creation failed for skill directory", {
          dir,
          err: innerErr,
        });
        return null;
  ...
  private filterByToolPolicy(
    ...
    if (
      ...
      availableTools.has(normalizeToolKey("read_skill"))
    ) {
      return skills;
    }
    ...
    return skills.filter((skill) => {
      ...
      return candidateKeys.some((key) =>
        availableTools.has(normalizeToolKey(key)),
      );
    });
  }
  ...
  private async getCachedOrReadSkill(location: string): Promise<string | null> {
    ...
    try {
      ...
      log.debug("failed to read cached skill file", { location, err });
      return null;
    }
  }
  ...
  private async meetsRequirements(skill: SkillMeta): Promise<boolean> {
    ...
    if (
      skill.requires.anyBins.length > 0 &&
      !(await this.hasAnyBinary(skill.requires.anyBins))
    ) {
    ...
    const missingConfigs = await this.findMissingConfigPaths(
      skill.requires.configs,
    );
  ...
  private async findMissingConfigPaths(
    configPaths: string[],
  ): Promise<string[]> {
    const missing: string[] = [];
  ...
  private async binaryExists(bin: string): Promise<boolean> {
    ...
    const hasKnownExtension =
      process.platform === "win32" && /\.[^./\\]+$/.test(raw);
  ...
  private async resolveAllowedSkillFilePath(
    location: string,
  ): Promise<string | null> {
  ...
  private async isWithinDirectoryRealPath(
    targetPath: string,
    allowedDir: string,
  ): Promise<boolean> {

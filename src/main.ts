...
const mode = process.argv.includes("--mode")
  ...
  : "text";
const workspaceDir =
  process.env.EDITH_WORKSPACE ?? path.resolve(process.cwd(), "workspace");
    ...
    try {
      ...
      if (pendingFeedback && pendingFeedback.memoryIds.length > 0) {
        ...
        const reward = Math.max(
          pendingFeedback.provisionalReward,
          followupReward,
        );
        ...
        void memory
          ...
          .catch((error) =>
            log.warn("async memrl feedback update failed", error),
          );
...
async function start(): Promise<void> {
  ...
  if (mode === "gateway" || mode === "all") {
    ...
    log.info("gateway ready", {
      ws: `ws://127.0.0.1:${config.GATEWAY_PORT}`,
      webchat: `http://127.0.0.1:${config.WEBCHAT_PORT}`,
    });
  }

  if (mode !== "text") {
    log.info("runtime status", {
      channels: channelManager.getConnectedChannels().join(", ") || "none",
